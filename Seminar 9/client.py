...
while True:
    ...
    if comanda == 'EXIT':
        ...
        if len(parti) < 2 or not parti[1].strip():
            print("EROARE: Te rog introdu un mesaj dupa comanda PUBLISH")
            continue
        raspuns = trimite_comanda(intrare)
        print(raspuns)
        ...
        if len(parti) < 2 or not parti[1].strip().isdigit():
            print("EROARE: Te rog introdu un ID numeric valid dupa comanda DELETE")
            continue
        raspuns = trimite_comanda(intrare)
        print(raspuns)
        ...
        if not este_conectat:
            print("EROARE: Trebuie sa fii conectat pentru a vedea mesajele!")
            continue
        raspuns = trimite_comanda(intrare)
        print(raspuns)
